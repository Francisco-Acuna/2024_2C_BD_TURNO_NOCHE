...
select 	distinct (s.documento), nombre, domicilio
from 	socios s
...
-- 5- Lista de socios (documento, nombre, domicilio) que tienen libros sin devolver y 
...
select 		count(*) cantidad_libros_no_devueltos
from 		libros l
join		prestamos p on l.codigo=p.codigo_libro
where 		fecha_devolucion is null;
...
select 	count(*) 'libros prestados este mes'
from	libros l 
join	prestamos p on l.codigo=p.codigo_libro
...
select current_date(); -- ANSI

select now(); -- no ANSI
select current_timestamp(); -- ANSI

-- 9- Cantidad de socios que tomaron libros prestados este mes
select 		distinct(count(s.documento))
from		socios s
join		prestamos p on s.documento=p.documento
where		fecha_prestamo between '2024-11-01' and '2024-11-30';
